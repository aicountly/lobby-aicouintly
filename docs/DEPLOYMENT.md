...
| --- | --- | --- | --- |
| Deploy to cPanel Production | `web/dist/` then `server-php/` | `<remote root>/` and `<remote root>/api/` | https://lobby.aicountly.com (+ `/api`) |
| Deploy to cPanel Sandbox | `web/dist/` then `server-php/` | `<remote root>/` and `<remote root>/api/` | https://lobby.gh.aicountly.com (+ `/api`) |
...
`https://lobby.aicountly.com/api/.env` unless Apache is told otherwise.
